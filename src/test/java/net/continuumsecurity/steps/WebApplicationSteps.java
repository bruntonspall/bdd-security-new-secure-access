...
public class WebApplicationSteps {
    ...
    public void changeCaseOfPassword()  throws IOException {
        if (World.getInstance().getCredentials() == null || World.getInstance().getUserPassCredentials().getPassword() == "") {
          setDefaultCredentials(Config.getInstance().getDefaultCredentials());
        }
        ...
        if (wrongCasePassword.equals(credentials.getPassword())) {
            ...
            if (wrongCasePassword.equals(credentials.getPassword())) {
                throw new RuntimeException(
                        "Password '"+credentials.getPassword()+"' doesn't have alphabetic characters, can't run this test.");
            } else {
